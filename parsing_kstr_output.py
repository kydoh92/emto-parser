...
def check_category(string, category):
	str = string[0:11].strip().split(':')[0]
	if str != category:
		print(" ### Error : Category mismatch; category = "+category+", input = "+str+"\n"); sys.exit()
	return string[11:]
...
def PRIMV(clines):
	check_category(clines.pop(0), 'PRIMV')
...
def PRIMKR(clines):
	str0 = check_category(clines.pop(0), 'PRIMKR')
...
def LATT3D(clines, NQ):
	str0 = check_category(clines.pop(0), 'LATT3D')
	NGHBP = ut.e1f(str0)
...
def BLATTS(clines, NQ):
	arr = np.empty((NQ, 8))
	for i in range(NQ):
		str0 = check_category(clines.pop(0), 'BLATTS')
...
def SCREEN(clines, NQ, NL):
	check_category(clines.pop(0), 'SCREEN')
	del clines[0:1]
	AWR = list(map(lambda x: tokenizer(clines.pop(0), even=0)[1:],range(NQ)))
	return AWR

def MGAUNT(clines):
	str0 = check_category(clines.pop(0), 'MGAUNT')
	NGAUNT, NGAUNTW = tokenizer(str0)
	return NGAUNT, NGAUNTW

def LATT3M(clines):
	str0 = check_category(clines.pop(0), 'LATT3M')
...
def MADL3D(clines, NQ):
	check_category(clines.pop(0), 'MADL3D')
...
def STORESH(clines, NQ):
	for i in range(NQ):
		check_category(clines.pop(0), 'STORES')
		check_category(clines.pop(0), 'STOREH')
	return 0

def END(clines):
	if len(clines) > 1:
		print(" ### Error : Too many lines left \n"); sys.exit()
	check_category(clines.pop(0), 'KSTR')
	return 0
#### MAIN ####
